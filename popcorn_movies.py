...
class PopcornClient:
    ...
    @classmethod
    def get_popcorn_movies(cls, page, sort='last updated'):
    ...
    def get_new_movies(cls, max_pages = 10, request_delay = 3):
        ...
        print('All movies in range were new, '
...
class MoviesStore:
    ...
    def add_rt_fields(movies, scrape_delay=3, overwrite=True):
        for m in movies:
            if overwrite or 'rotten_tomatoes' not in m:
                m.update({'rotten_tomatoes': RTScraper(m['title']).get_ratings()})
        ...
        for filename in files:
            ...
            with open(filepath, 'rt') as f:
                try:
                    ...
                    print(f'Failed json read for {filepath}')
    ...
    def get_new_movies(self, max_pages=3):
        new_movies = PopcornClient.get_new_movies(max_pages=max_pages)
        self.add_rt_fields(new_movies)
        self.save_movies(new_movies)
        return new_movies
